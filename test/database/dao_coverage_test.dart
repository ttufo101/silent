...
void main() {
  ...
  test(
    ...
    () async {
      const first = Profile(
        ...
        selectedMap: {'Selector': 'Proxy'},
        overwriteType: OverwriteType.custom,
        scriptId: 7,
      );
      const second = Profile(id: 2, label: 'Second');
      ...
      expect(profiles.map((profile) => profile.id), [1, 2]);
      expect(profiles.first, first);
      ...
      final replacement = first.copyWith(label: 'Replaced');
  ...
  test('database restore and custom data replace related records', () async {
    ...
    expect(await database.profilesDao.query().get(), [profile]);
