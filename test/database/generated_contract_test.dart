...
void main() {
  test('profile and script generated data classes preserve all fields', () {
    ...
    const profile = RawProfile(
      ...
      scriptId: 2,
      selectedMap: {'Select': 'DIRECT'},
    );

    expect(profile.toColumns(true), hasLength(6));
    expect(profile.toCompanion(true).toColumns(true), hasLength(6));
    ...
    expect(emptyProfile.toColumns(true), hasLength(4));
    expect(emptyProfile.toColumns(false), hasLength(6));
    expect(emptyProfile.toCompanion(true).toColumns(true), hasLength(4));

    final insertedProfile = ProfilesCompanion.insert(
      label: 'Inserted',
      overwriteType: OverwriteType.script,
      selectedMap: const {},
    ).copyWith(id: const Value(8));
    expect(insertedProfile.toColumns(true), hasLength(4));
